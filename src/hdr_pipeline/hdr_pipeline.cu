...
#include "stdio.h"
#include "math.h"
__device__ float compute(float input_tone, int width, int height);
...
__device__ float compute(float input_tone, int width, int height){
	...
	float output_tone =  input_tone*(0.9036*input_tone + 0.018)/(input_tone*(0.8748*input_tone+0.354)+0.14);
	if(output_tone <= 0.0031308)
		output_tone = 12.92 * output_tone;
	else
		output_tone = 1.055 * powf(output_tone,1/2.4) - 0.055;
	output_tone = (output_tone>1) ? 255 : rintf(255*output_tone);
	return output_tone;
