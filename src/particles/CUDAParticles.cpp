...
CUDAParticles::CUDAParticles(std::ostream& output_file, particle_system_module& module, std::size_t num_particles, std::unique_ptr<float[]> position, std::unique_ptr<std::uint32_t[]> color, const ParticleSystemParameters& params)
	: particles(module.create_instance(num_particles, &position[0] + 0 * num_particles, &position[0] + 1 * num_particles, &position[0] + 2 * num_particles, &position[0] + 3 * num_particles, &color[0], params)),
	  ...
	  writer(output_file, num_particles, &position[0] + 0 * num_particles, &position[0] + 1 * num_particles, &position[0] + 2 * num_particles, &position[0] + 3 * num_particles, &color[0], params)
{
}
...
float CUDAParticles::update(int steps, float dt)
{
	...
	writer.add_frame(steps * std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::duration<float>(dt)), position_download_buffer.get(), color_download_buffer.get());
